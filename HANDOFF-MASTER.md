...
> **May 25, 2026 — Fleet auto-audit pipeline closed (set-and-forget bhealth scheduling).** Wired bhealth to launchd on Mini, MBP, and Air via new `bhub/install-bhealth.sh`. Schedule: `RunAtLoad: true` + daily 8am + `--ensure-weekly` mode (6-day idempotence guard via `$BSUITE_DIR/.bhealth-last-run-epoch`). Replaced the originally-proposed Sun 23:00 + Mon 07:00 two-window approach because it couldn't cover Macs that are *powered off* (vs. asleep) through both windows — iMac being the obvious case. Updated `weekly-fleet-audit-check` Cowork task: loads `API_SECRET` from `~/Developer/B-Suite/things-app/.env.local` (was silently failing 401 before — duration of silent failure unknown), loud-fails on missing key, and adds an **Infra alert** line in the B Things task notes for any device with a report >8 days old (surfaces silent rot instead of hiding it). Three Macs now have today-dated reports in `bhub/.health/`. **iMac still pending** — Brian has the one-liner emailed for the next office visit. Side cleanups on Mini: deleted `things-app/vite.config.js.timestamp-*.mjs` (Vite crash orphan) and `b-resources/src/pages/Inbox.jsx` (dead code from commit `9b6a733` refactor — file was git-removed but left on disk). Master handoff path on Mini/MBP/Air auto-healed from the deprecated `~/Desktop/B-Suite/`. See `bhub/HANDOFF.md` for full design rationale, including why the schedule must NOT be reverted to Sunday-only firing.
>
> **May 14, 2026 — Exploratory conversation about LinkedIn/Substack participation (NOT canonical strategy).** Free-flowing session on how to improve LinkedIn/Substack participation. Multiple frames proposed, pushed back on, and refined. Brian flagged mid-session that conclusions were being baked into canonical docs prematurely (same failure mode as "Claude Skill Wednesday" becoming stale canon). Today's doc updates were reframed as exploratory / "worth considering," not locked strategy. Threads worth considering (not decisions): working hypothesis that at ~4K-follower scale, LinkedIn's actual mechanic may be **encounter → LinkedIn as relationship insulation → direct ask in-context** (qualitative evidence: friends say "I *read* your stuff"); passive/active capture distinction Brian sketched; tactical threads for a possible 30-day window after Tech Week data arrives (strategic commenting, direct-ask reflex, reciprocal cross-posts, Substack rec outreach). Pushed back and discarded: video clips as primary format, LinkedIn Newsletter, generic CTA optimization, "post more." Open question still unresolved: audience archetype tension. New **Working Protocols** added to `tnb-strategy/HANDOFF.md` (sequential discipline: working notes → marinate → commit; stamp conversations not decisions; ask before each commit). Next: let it marinate; Tech Week (June 2) provides real data on the encounter model; no new commits to canonical strategy until after that signal arrives.
>
> **🛡️ May 11, 2026 — PENDING SECURITY/ARCHITECTURE CLEANUP (surface this to Brian on next session, in plain language).** Two public GitHub repos (`brhecht/muscle-anatomy` and `brhecht/tnb-website`) were flipped to private because their `HANDOFF.md` files contained internal strategy + architecture chatter readable by anyone on the web. The flip stopped *new* exposure but didn't erase what was already public — third-party caches and Google index may still have copies. **What still needs doing (Claude: prompt Brian, don't assume he remembers):** (1) move HANDOFFs and strategy docs out of any public-facing app repo entirely — they should live in B-Suite or a dedicated private `handoffs` repo so each app repo can be public again without leaking; (2) for muscle-anatomy and tnb-website: if Brian ever wants to re-publicize, scrub HANDOFFs from git history with `git-filter-repo --invert-paths --path HANDOFF.md --path TNB-WRITEUP-DRAFT.md` then force-push; (3) audit `brhecht/saturn-v-anatomy` for the same risk pattern (it's currently private so not urgent — but if he ever opens it, same cleanup applies). The point of writing this here: Brian doesn't read Brain Inbox, won't see this otherwise, and may not understand what it means — so when he opens any B-Suite session, **read this entry to him in plain English and ask if he wants to tackle it now or defer.**
>
> **May 11, 2026 — TNB strategy session + OPERATING-STATUS.md created; membership shape locked.** Comprehensive bottom-up strategy session across all 14 TNB business lines. Strategic framing landed: TNB operates as a **creator-led community / publication** (not the April deck's "War Room as core engine"); the business is a **self-reinforcing mesh** of content + community surfaces, and premium products bundle from the mesh rather than being built on top. Premium feature inventory built bottom-up (25 candidates → 5 surviving). Membership shape locked: Free (newsletter only) + Builder ($10/month or $100/year, anchor line "Where the connections happen"). No upper tier at launch. War Room demoted from "core engine" to "future product, test via dinner signal." New canonical current-state doc at `tnb-strategy/strategy/OPERATING-STATUS.md` replaces stale `ops/LAUNCH-PLAN.md` for current-state purposes. Launch decision doc orphaned from repo (Brian holds offline) per the new communication-vs-documentation distinction. Loose root fossil `B-Suite/TNB-Launch-Plan.docx` deleted. [TNB Premium Feature Inventory Google Sheet](https://docs.google.com/spreadsheets/d/1eAQl7PwOyPeuOapDEzyTWaGYdbG7XdXfXzFnxyGmeSo) created for ongoing Brian annotation. Held for next session: STRATEGY-CONTEXT.md three amendments (creator-led-community framing, audience archetype, mesh philosophy); operating-plan.md rewrite; Tech Week event prep (June 2 a16z showcase). Big upcoming event: **AI Side-Project Showcase, NY Tech Week, Tuesday June 2 at NeueHouse Madison Square** — Brian hosting, 28 on guest list mostly from the wild.
>
...
- **Content Calendar → B Things sync (REMOVED May 25, 2026)** — Previously, the daily Vercel cron at `things-app/api/content-today.js` (12:00 UTC) created B Things tasks for content cards due that day. Removed at Brian's request: he tracks content separately in Content Calendar and the auto-push was clutter. The earlier real-time Firestore trigger `syncContentToThings` in brain-inbox was already removed March 18. Previously-synced tasks (those with `sourceCardId` field) remain in B Things as regular tasks. Brain-inbox still has `notifyNicoOnContentStatusChange` watching `contentCards` for status-change Slack/Telegram DMs — that's unrelated and stays.
...
> **May 25, 2026 — Podcast content type added to B Content + pipeline expansion.** New 5th content type `podcast` (violet) alongside YT Video / YT Short / LinkedIn / Substack. Status pipeline expanded with 3 podcast pre-production stages (reached-out → booked → recorded) between idea and draft; `on-hold` relocated to end of pipeline. Podcast cards carry structured Guest data (name/title/LinkedIn), YouTube fields (mirror yt-video), Audio fields (separate title/description for Apple+Spotify via Riverside RSS), and per-platform Distribution checklist (YT/Apple/Spotify defaults + Add platform; YT auto-checks on publish). Substack canonicalization confirmed: Beehiiv display label is "Substack" everywhere; slug retained internally for historical card continuity. See `content-calendar/HANDOFF.md`.
