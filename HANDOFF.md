...
### 2026-05-07 — Master handoff split + handoff skill v3.6.0
- **What shipped:** `HANDOFF-MASTER.md` slimmed from 50KB / 527 lines to 17KB / 252 lines: dropped the chronological journal (24 entries → moved to new `HANDOFF-HISTORY.md`) and collapsed the 14 per-app sections into a 1-line-per-app Apps Index table. Cross-cutting reference sections (Cross-Project Dependencies, UX Standards, bhealth, Status Recap Rules, Notification Routing, User Registry, Devices, Local Paths, Known Issues, Backlog) preserved verbatim. Per-app deep detail stays in each repo's own `HANDOFF.md` — that's the source of truth. Handoff skill v3.5.0 → 3.6.0: skill prompt now loads master only at bootstrap; HISTORY is on-demand. Combined with v3.5.0's lazy bootstrap + bsync v2.6's --app= flag, a typical "handoff here" should drop from 2-5 min to 30-60s.
- **Known issues:** All devices will see handoff-skill mismatch on next handoff and need a one-click reinstall (same flow as today's earlier v3.5.0 push — they'll go straight from 3.4.0 to 3.6.0).
- **Next:** Measure end-to-end speedup in next session. Future trims: per-app HANDOFF.md files probably also have stale sections worth pruning, but lower priority.
